...
global net
net = caffe.Classifier('deploy.prototxt', 'CASIA_iter_666000.caffemodel')
...
if __name__ == '__main__':
    ...
    for thershold in np.arange(0.25, 0.65, 0.01):
        True_Positive = 0
        True_Negative = 0
    	False_Positive = 0
    	False_Negative = 0
        ...
        for index in range(len(PositiveDataList)):
            print PositiveDataList[index].split(' ')
            filepath_1 = PositiveDataList[index].split(' ')[0]
            filepath_2 = PositiveDataList[index].split(' ')[1][:-2] #'Abel_Pacheco/Abel_Pacheco_0004.jpg\n'
